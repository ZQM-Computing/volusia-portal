"""Add metadata footer — footer already has closing tags, no extra wrapper."""
import os

base = r'C:\Users\zqmco\Docker\volusia-portal'

# Footer ends with </footer> — the component closing )\n} comes after
footer_jsx = """
...
def add_footer(fname):
    ...
    # Replace the component-closing block with footer + closing
    old = "    </div>\n  )\n}"
    assert old in data, f"{fname}: closing block not found"
    # The footer JSX already ends with </footer>; then we add the component closing
    new = "    </div>" + footer_jsx + "\n  )\n}"
