"""Add metadata footer to HomePage.tsx — correct closing."""
import os
...
# HomePage closing: `    </div>\n  )\n}` — but wait, the footer adds extra closing tags
# The footer ends with `    </footer>` then `  )\n}`
# So we need to find `    </div>\n  )\n}` and replace with `    </div>` + footer
old = "    </div>\n  )\n}"
