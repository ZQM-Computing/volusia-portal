"""Add metadata footers — precise fix for BusinessPage and TouristsPage."""
import os
...
# BusinessPage - replace closing block
f = os.path.join(base, 'src/pages/BusinessPage.tsx')
with open(f, 'rb') as fh:
    data = fh.read().decode('utf-8').replace('\r\n', '\n')
# BusinessPage has 3 closing divs (tools grid + card + page), then `)` then `}`
old = "      </div>\n    </div>\n  )\n}"
new = "      </div>\n    </div>\n\n" + footer + "\n  )\n}"
